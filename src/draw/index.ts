...
export default function draw(w: number, h: number, canvas: HTMLCanvasElement, options: Required<OptionsType>, store: ElementStoreType) {
    try {
        if (!options.custom)
            import(`./model/${options.model}.ts`).then(model => {
                new model.default(w, h, canvas, options, store.element.$store)
            })
        else new options.custom(w, h, canvas, options, store.element.$store)
    } catch (e) {
