...
export default class Skeleton extends BaseModel<Required<SkeletonOptionsType>> {
    ...
    initPoint() {
        ...
        // 动画结束回调(还原样式)
        this.store.colseCall = () => {
            this.skeleton.forEach(s => {
                let el = s.element
                if (el.getAttribute(this.WL_KEY)) {
                    let wl_key = el.getAttribute(this.WL_KEY)
                    if (wl_key) el.style.cssText = wl_key
    ...
    controller(els: HTMLCollection) {
        ...
        for (let e of Array.from(els)) {
            ...
            else if (op.deep) this.controller(e.children)
            else this.skeleton.push({ title: e.nodeName, element: e as HTMLElement })
        }
    }
    drawSkeleton() {
        let op = this.options
        ...
        this.skeleton.forEach(s => {
            ...
            if (this.store.element.$store?.animationId) {
                let w = el.getAttribute(this.WL_W), h = el.getAttribute(this.WL_H);
                if (w || h) {
                    if (!el.getAttribute(this.WL_KEY))
                    ...
                    if (op.skeletonMax) {
                        ...
                        if (h && el.offsetHeight > parseInt(h)) w = String(el.offsetHeight)
                    }
                    el.style.cssText = `
                            ${el.style.cssText}
